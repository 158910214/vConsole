...
module.exports = (env, argv) => {
  ...
  const __TARGET__ =
    BuildTarget.indexOf(env.target) === -1 ? BuildTarget[0] : env.target;
  ...
  if (noCoreJS) {
    ...
    plugins.push(
      new Webpack.NormalModuleReplacementPlugin(/^core-js\/.*/, dummyModulePath)
    );
  }
  ...
  return {
    ...
    resolve: {
      extensions: [".ts", ".js", ".html", ".less", ".mjs", ".svelte", "png"],
      alias: {
          ...
          use: [
            {
              loader: "url-loader",
              options: {
                limit: 100 * 1024, // 限制文件大小（以字节为单位），小于该大小的文件会被转为Base64
